...
from FeedServer.market import Kline,Orderbook,Trade
from FeedServer.decorator import async_method_locker
...
class Event:
    ...
    def subscribe(self, callback, multi=False):
        ...
        """
        from FeedServer.quant import quant
    ...
    * NOTE:
    ...
    def __init__(self, platform=None, symbol=None, open=None, high=None, low=None, close=None, volume=None,
        ...
        if kline_type == const.MARKET_TYPE_KLINE:
            ...
            log.error("kline_type error! kline_type:", kline_type, caller=self)
            return
...
class EventCenter:
    ...
    async def subscribe(self, event: Event, callback=None, multi=False):
        ...
        """
        log.info("NAME:", event.name, "EXCHANGE:", event.exchange, "QUEUE:", event.queue, "ROUTING_KEY:",
    ...
    async def publish(self, event):
        ...
        if not self._connected:
            log.warn("RabbitMQ not ready right now!", caller=self)
            return
        data = event.dumps()
    ...
    async def connect(self, reconnect=False):
        ...
        """
        log.info("host:", self._host, "port:", self._port, caller=self)
        if self._connected:
            return
        ...
        try:
            ...
            log.error("connection error:", e, caller=self)
            return
        finally:
            if self._connected:
                return
        ...
        log.info("Rabbitmq initialize success!", caller=self)
        ...
        log.debug("create default exchanges success!", caller=self)
    ...
    async def _initialize(self, event: Event, callback=None, multi=False):
        ...
        if callback:
            if multi:
                ...
                log.info("multi message queue:", queue_name, caller=self)
            else:
                await self._channel.basic_consume(self._on_consume_event_msg, queue_name=queue_name)
                log.info("queue:", queue_name, caller=self)
    ...
    def _add_event_handler(self, event: Event, callback):
        ...
        log.debug("event handlers:", self._event_handler.keys(), caller=self)

    async def _check_connection(self, *args, **kwargs):
        if self._connected and self._channel and self._channel.is_open:
            log.debug("RabbitMQ connection ok.", caller=self)
            return
        log.error("CONNECTION LOSE! START RECONNECT RIGHT NOW!", caller=self)
