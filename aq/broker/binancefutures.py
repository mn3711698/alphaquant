...
class BinanceFutures(BaseBroker):
    ...
    def buy(self, symbol, quantity, price=None, tif=Timeinforce.GTC):
        if price:
            order = self.create_order(BUY, symbol, quantity, price, tif=tif)
        else:
            order = self.create_order(BUY, symbol, quantity, order_type=OrderType.MARKET)

        return self.parse_order(order)

    def sell(self, symbol, quantity, price=None, tif=Timeinforce.GTC):
        if price:
            order = self.create_order(SELL, symbol, quantity, price, tif=tif)
        else:
    ...
    def close_position(self, position):
        if position.direction == Direction.LONG:
            return self.sell(position.symbol, abs(position.qty))
        else:
            return self.buy(position.symbol, abs(position.qty))
